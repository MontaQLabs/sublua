...
-- Find the absolute path to the app bundle's Contents directory
local function get_app_bundle_path()
    local source = love.filesystem.getSource()
    
    -- Check if we're running from a .app bundle
    -- source will be like: /path/to/App.app/Contents/Resources/game.love
    -- OR: /path/to/App.app/Contents/Resources (if fused directly)
    if source:match("%.app/Contents/Resources") then
        -- Remove the .love filename if present
        local resourcesPath = source:gsub("/[^/]+%.love$", "")
        -- Now we have: /path/to/App.app/Contents/Resources
        -- Remove /Resources to get: /path/to/App.app/Contents
        local contentsPath = resourcesPath:gsub("/Resources$", "")
        return contentsPath
    end
    
    -- Running as standalone .love or from directory
    return nil
end
...
function Blockchain.init()
    ...
    local appBundlePath = get_app_bundle_path()
    local sourceDir = love.filesystem.getSource()
    local home = os.getenv("HOME") or ""
    
    -- Set up package.path for SubLua modules
    package.path = "./?.lua;./?/init.lua;./sublua/?.lua;sublua/?.lua"
    package.path = package.path .. ";" .. sourceDir .. "/?.lua"
    package.path = package.path .. ";" .. sourceDir .. "/sublua/?.lua"
    ...
    -- Check if sublua directory exists in love filesystem
    local info = love.filesystem.getInfo("sublua")
    if info and info.type == "directory" then
        print("✓ Found bundled SubLua in .love file")
    end
    
    -- Load SubLua module
    local ok, mod = pcall(require, "sublua")
    if not ok then
        print("⚠ SubLua not found - demo mode")
        print("  Error: " .. tostring(mod))
        return
    end
    
    sublua = mod
    print("✓ SubLua module loaded")
    
    -- Build list of FFI library paths to try (ABSOLUTE paths!)
    local ffi_paths = {}
    
    -- 1. If running from .app bundle, check Frameworks directory
    if appBundlePath then
        table.insert(ffi_paths, appBundlePath .. "/Frameworks/libpolkadot_ffi.dylib")
        table.insert(ffi_paths, appBundlePath .. "/MacOS/libpolkadot_ffi.dylib")
        table.insert(ffi_paths, appBundlePath .. "/Resources/precompiled/macos-aarch64/libpolkadot_ffi.dylib")
    end
    
    -- 2. User's SubLua installation
    if home ~= "" then
        table.insert(ffi_paths, home .. "/.sublua/lib/libpolkadot_ffi.dylib")
    end
    
    -- 3. Relative to source directory (for development)
    table.insert(ffi_paths, sourceDir .. "/precompiled/macos-aarch64/libpolkadot_ffi.dylib")
    table.insert(ffi_paths, "./precompiled/macos-aarch64/libpolkadot_ffi.dylib")
    table.insert(ffi_paths, "precompiled/macos-aarch64/libpolkadot_ffi.dylib")
    
    -- 4. System paths
    table.insert(ffi_paths, "/usr/local/lib/libpolkadot_ffi.dylib")
    table.insert(ffi_paths, "/opt/homebrew/lib/libpolkadot_ffi.dylib")
    
    -- Try each path
    local ffi_loaded = false
    for _, path in ipairs(ffi_paths) do
        local success = pcall(function() sublua.ffi(path) end)
        if success then
    ...
    if ffi_loaded then
        sublua_loaded = true
        print("✓ SubLua FFI loaded - LIVE blockchain mode")
        GameState.blockchain_ready = true
    else
        print("⚠ FFI library not found - demo mode")
        print("  Tried " .. #ffi_paths .. " paths")
        if appBundlePath then
