...
/* Seed-based keypair: randombytes copies from global seed buffer */
static const unsigned char *_tweetnacl_seed_ptr = 0;

void randombytes(unsigned char *x, unsigned long long n)
{
  unsigned long long i;
  if (_tweetnacl_seed_ptr) {
    FOR(i, n) x[i] = _tweetnacl_seed_ptr[i];
  } else {
    FOR(i, n) x[i] = 0;
  }
}

void tweetnacl_keypair_from_seed(unsigned char *pk, unsigned char *sk, const unsigned char *seed)
{
  _tweetnacl_seed_ptr = seed;
  crypto_sign_keypair(pk, sk);
  _tweetnacl_seed_ptr = 0;
}
